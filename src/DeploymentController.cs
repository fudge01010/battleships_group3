...
static class DeploymentController
{
	...
	public static void HandleDeploymentInput()
	{
		...
		if (SwinGame.MouseClicked(MouseButton.LeftButton)) {
			...
			//fixed error here, first elseif also had Direction.LeftRight, now fixed
	...
	private static void DoDeployClick()
	{
		...
		int row = 0;
		int col = 0;
		//added - field_top, fixed the mouse positioning
	...
	public static void DrawDeployment()
	{
		...
		foreach (ShipName sn in Enum.GetValues(typeof(ShipName))) {
			...
			if (i >= 0) {
				if (sn == _selectedShip) {
					...
					SwinGame.DrawRectangle(Color.LightBlue, SHIPS_LEFT, SHIPS_TOP + i * SHIPS_HEIGHT, SHIPS_WIDTH, SHIPS_HEIGHT);
					SwinGame.DrawText(sn.ToString(), Color.White, GameFont("Menu"), SHIPS_LEFT + TEXT_OFFSET, SHIPS_TOP + i * SHIPS_HEIGHT);
				} else {
					SwinGame.DrawRectangle(Color.Gray, SHIPS_LEFT, SHIPS_TOP + i * SHIPS_HEIGHT, SHIPS_WIDTH, SHIPS_HEIGHT);
					SwinGame.DrawText(sn.ToString(), Color.Gray, GameFont("Menu"), SHIPS_LEFT + TEXT_OFFSET, SHIPS_TOP + i * SHIPS_HEIGHT);
				}
			}
		}
		...
		if (HumanPlayer.ReadyToDeploy) {
			...
			//SwinGame.DrawText("PLAY", Color.White, GameFont("Menu"), PLAY_BUTTON_LEFT + TEXT_OFFSET, TOP_BUTTONS_TOP);
		}

		SwinGame.DrawBitmap(GameImage("RandomButton"), RANDOM_BUTTON_LEFT, TOP_BUTTONS_TOP);
		SwinGame.DrawText("Shuffle", Color.White, GameFont("Menu"), RANDOM_BUTTON_LEFT + TEXT_OFFSET, TOP_BUTTONS_TOP);
